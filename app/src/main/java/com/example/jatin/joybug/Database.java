...
public class Database {
    ...
    public boolean addDriver(Driver setDriver) {
        try {
            String email = setDriver.getEmail();
            String driveDate = setDriver.getDepartDate();
            this.database.getReference().child("users").child("drivers").child(email).setValue(setDriver);
            // thinking of having two tables, one for user data one for drive data info
            this.database.getReference().child("driveData").child(driveDate).child(email).setValue(setDriver);
            return true;
        } catch (Exception e) {
            return false;
        }
    ...
    public boolean addCarpooler(Carpooler setCarpooler) {
        try {
            String netid = setCarpooler.getEmail();
    ...
    public String[] findDriver(String email) {
        try {
            DatabaseReference ref = this.database.getReference().child("users").child("drivers").child(email);
            ...
            String[] output = {email, name, destination, bio, departDate};
            return output;
        } catch (Exception e) {
    ...
    public String[] findDriveDate(String findDriveDate) {
        ...
        try {
            DatabaseReference ref = this.database.getReference().child("findDriveData")
                    .child(findDriveDate);
            String name = ref.child("name").getKey();
            String netid = ref.child("email").getKey();
            String destination = ref.child("destination").getKey();
            String bio = ref.child("bio").getKey();
            String departDate = ref.child("departDate").getKey();
            String[] output = {netid, name, destination, bio, departDate};
